...
module.exports = function transformer(file, api, options) {
  ...
  if (config.hoist === true) {
    ...
    var lastImport =
      lastImportIndex >= 0
        ? root.find(j.Program).get("body", lastImportIndex)
        : undefined
    ...
    getImportExpressionStatements(j, root)
      ...
      .forEach(function (expressionStatement) {
        var newExpression = j.expressionStatement(
  ...
  root
    .find(j.VariableDeclaration, {
      declarations: [{ init: { type: "ObjectExpression" } }],
    })
    .forEach(function (variableDeclaration) {
      ...
      j(variableDeclaration)
        ...
        .forEach(function (property) {
          ...
          var importStatement = util.createImportStatement(
            moduleName,
            variableName,
            undefined,
          ...
          // modify property
          var newProp = api.jscodeshift.property(
            property.node.kind,
            property.node.key,
            property.node.key,
          )
...
function getCalledImportDeclarators(j, rootAst) {
  ...
  return rootAst
    ...
    .filter(function (vdRef) {
      ...
      return (
        "arguments" in callExpression && Array.isArray(callExpression.arguments)
      )
    })
}
...
function replaceDeclarator(j, variableDeclarator, index) {
  var variableDeclaration = variableDeclarator.parent
  var variableDeclarationComments = Array.isArray(
  ...
  var comments = variableDeclarationComments.filter(function (comment) {
    return (
      (isFirstDeclarator && comment.leading) ||
      (isLastDeclarator && comment.trailing)
    )
  })
  ...
  if (isLastDeclarator) {
    ...
    j(variableDeclaration).insertBefore(
      createRequire(j, varStatement, comments),
    )
...
function createIntermediateImport(j, variableDeclarator, comments) {
  ...
  var leadingComments = comments.filter(function (c) {
    return c.leading
  })
...
function createDeclaredCallExpression(j, variableDeclarator, comments) {
  ...
  var newDeclaration = j.variableDeclaration("const", [
    j.variableDeclarator(
      id,
      j.callExpression(j.identifier(factoryName), calledArgs),
    ),
  ])
  var trailingComments = comments.filter(function (c) {
    return c.trailing
  })
