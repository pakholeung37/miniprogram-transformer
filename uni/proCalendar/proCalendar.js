Component({
  ...
  methods: {
    ...
    getDateArrByYearAndMonth(year, month) {
      ...
      //目标月1号对应的星期
      let startWeek = getWeek(year, nextMonth, 1);
      //获取目标月有多少天
      ...
      for (let j = -startWeek + 1; j <= dayNums; j++) {
        ...
        let isOutDate = false;
        if (
          (j < DATE_DAY && year == DATE_YEAR && month + 1 == DATE_MONTH) ||
          (month + 1 < DATE_MONTH && year == DATE_YEAR) ||
          year < DATE_YEAR
        ) {
          //当天之前的日期不可用
          disabled = true;
    ...
    handleClickDate(e) {
      ...
      const disabled = dataset.disabled || 0;
      // 选择不可选择单元或置灰单元, 不做任何操作
      ...
      if(!this.data.multiSelect) {
        ...
        if (
          !isADate(selectEnd) 
          && isADate(selectStart) 
          && greaterThan(selected, selectStart) 
          /** 检查区间中是否有disabled */
    ...
    isDateRangeHasDisabled(start, end) {
      if(!this.data.timeMapData) return false
      // 直接穷举出所有选中日期
      const arr = [];
...
function toDate({year, month, date}) {
  return new Date(year, month - 1, date);
}

function isADate({year, month, date}) {
